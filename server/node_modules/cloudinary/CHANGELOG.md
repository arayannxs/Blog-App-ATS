...
* Add support `max_results` and `next_cursor` in `***REDACTED***_folders` and `sub_folders` (#411)
...
==================
  ...
  * remove duplicate object key
  * Support ***REDACTED*** path for shared CDN
...
===================
  ...
  * - added ***REDACTED***_folders & sub_folders management api + tests - fixed v2 module (requiring v2 would override v1) - fixed api promises reject a result with error attribute - added dotenv for test environment
  * ignoring bin folder
  * updated demo
  * node photo album
