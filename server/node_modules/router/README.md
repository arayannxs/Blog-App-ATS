...
given `path`, defaulting to the ***REDACTED*** path.
...
You can get the message by running this command in your terminal,
 or navigating to `***REDACTED***:8080` in a web browser.
```bash
curl http://***REDACTED***:8080
```

You can set the message by sending it a `PATCH` request via this command:
```bash
curl http://***REDACTED***:8080/api/set-message -X PATCH -H "Content-Type: application/json" -d '{"value":"Cats!"}'
```
...
curl http://***REDACTED***:8080
> kitten
curl http://***REDACTED***:8080/such_path
> such_path
```
