...
function getTsConfigDefaults(config, basePath, _files, _include, _exclude) {
    ...
    let ***REDACTED***Dir = config.options.***REDACTED***Dir;
    if (***REDACTED***Dir == null) {
        if (composite)
            ***REDACTED***Dir = basePath;
        // Return this symbol to avoid computing from `files`, which would require fs calls
        else
            ***REDACTED***Dir = exports.ComputeAsCommonRootOfFiles;
    }
    const { outDir = ***REDACTED***Dir } = config.options;
    ...
    return { ***REDACTED***Dir, outDir, include, files, exclude, composite };
}
