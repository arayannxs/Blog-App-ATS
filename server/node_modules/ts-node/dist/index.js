...
function createFromPreloadedConfig(foundConfigResult) {
    ...
    const scopeDir = (_c = (_b = (_a = options.scopeDir) !== null && _a !== void 0 ? _a : config.options.***REDACTED***Dir) !== null && _b !== void 0 ? _b : configFileDirname) !== null && _c !== void 0 ? _c : cwd;
    ...
    if (!transpileOnly) {
        ...
        const ***REDACTED***FileNames = new Set(config.fileNames);
        const cachedReadFile = (0, util_1.cachedLookup)(debugFn('readFile', readFile));
        // Use language services by default
        if (!options.compilerHost) {
            let projectVersion = 1;
            const fileVersions = new Map(Array.from(***REDACTED***FileNames).map((fileName) => [fileName, 0]));
            ...
            const serviceHost = {
                ...
                getScriptFileNames: () => Array.from(***REDACTED***FileNames),
            ...
            const updateMemoryCache = (contents, fileName) => {
                // Add to `***REDACTED***Files` as necessary, either to make TS include a file it has not seen,
                // or to trigger a re-classification of files from external to internal.
                if (!***REDACTED***FileNames.has(fileName) &&
                    !isFileKnownToBeInternal(fileName)) {
                    markBucketOfFilenameInternal(fileName);
                    ***REDACTED***FileNames.add(fileName);
                    // Increment project version for every change to ***REDACTED***FileNames.
                    projectVersion++;
                }
            ...
            let builderProgram = ts.createIncrementalProgram
                ? ts.createIncrementalProgram({
                    ***REDACTED***Names: Array.from(***REDACTED***FileNames),
                    options: config.options,
                    host,
                    configFileParsingDiagnostics: config.errors,
                    projectReferences: config.projectReferences,
                })
                : ts.createEmitAndSemanticDiagnosticsBuilderProgram(Array.from(***REDACTED***FileNames), config.options, host, undefined, config.errors, config.projectReferences);
            ...
            const updateMemoryCache = (contents, fileName) => {
                ...
                // Add to `***REDACTED***Files` when discovered by compiler for the first time.
                ...
                if (addedToRootFileNames || contentsChanged) {
                    builderProgram = ts.createEmitAndSemanticDiagnosticsBuilderProgram(Array.from(***REDACTED***FileNames), config.options, host, builderProgram, config.errors, config.projectReferences);
                }
            };
            getOutput = (code, fileName) => {
                let outText = '';
