...
export function extractDefs<T extends schemas.$ZodType>(
  ...
  const ***REDACTED*** = ctx.seen.get(schema);

  if (!***REDACTED***) throw new Error("Unprocessed schema. This is a bug in Zod.");
  ...
  const extractToDef = (entry: [schemas.$ZodType<unknown, unknown>, Seen]): void => {
    ...
    // or if the schema is the ***REDACTED*** schema
  ...
  for (const entry of ctx.seen.entries()) {
    ...
    // convert ***REDACTED*** schema to # $ref
    if (schema === entry[0]) {
      extractToDef(entry); // this has special handling for the ***REDACTED*** schema
      continue;
    }
...
export function finalize<T extends schemas.$ZodType>(
  ...
  Object.assign(result, ***REDACTED***.def ?? ***REDACTED***.schema);

  // The `id` in `.meta()` is a Zod-specific registration tag used to extract
  // schemas into $defs — it is not user-facing JSON Schema metadata. Strip it
  // from the output body where it would otherwise leak. The id is preserved
  // implicitly via the $defs key (and via $ref paths).
  const ***REDACTED***MetaId = ctx.metadataRegistry.get(schema)?.id;
  if (***REDACTED***MetaId !== undefined && result.id === ***REDACTED***MetaId) delete result.id;
