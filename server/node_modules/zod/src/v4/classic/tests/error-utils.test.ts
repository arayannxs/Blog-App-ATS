...
test("z.formatError nested union preserves parent path", () => {
  ...
  // "child" must be nested under "parent", not at ***REDACTED***
...
test("z.treeifyError nested union preserves parent path", () => {
  ...
  // the inner errors must stay nested under their parent path, not flatten to ***REDACTED***.
  const syntheticError = new z.ZodError([
    {
      code: "invalid_union",
      path: ["parent"],
...
test("z.treeifyError nested union with real schema", () => {
  ...
  if (!result.success) {
    ...
    // All settings-related errors should be under "settings", not at ***REDACTED***
    ...
    for (const key of Object.keys(settingsProperties)) {
      // Every sub-property under settings should NOT also appear at ***REDACTED***
      if (key !== "settings") {
